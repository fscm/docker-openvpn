...
```
docker container run --volume <OPENVPN_VOL>:/data:rw --rm <USER>/<IMAGE>:<TAG> [options] init
```
...
```
docker container run --volume my_openvpn:/data:rw --rm johndoe/my_openvpn:latest -c vpn.johndoe.com -n 8.8.8.8 -n 8.8.4.4 -p 'route 10.69.0.0 255.255.0.0' -s 172.16.0.0/16 -u udp://vpn.johndoe.com:1194 init
```
...
```
docker container run --volume <OPENVPN_VOL>:/data:rw --detach --publish 1194:1194/udp --cap-add=NET_ADMIN --device=/dev/net/tun <USER>/<IMAGE>:<TAG> start
```
...
```
docker container run --volume my_openvpn:/data:rw --detach --publish 1194:1194/udp --cap-add=NET_ADMIN --device=/dev/net/tun --name my_openvpn johndoe/my_openvpn:latest start
```
